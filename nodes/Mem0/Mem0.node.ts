			...
			try {
				...
				if (!userId || !agentId) {
					throw new NodeOperationError(
						this.getNode(),
						'User ID and Agent ID are required for this safe profile.',
					);
				}
				...
				if (operation === 'storeInteraction') {
					...
					if (userMessage.length > MAX_MESSAGE_LENGTH) {
						throw new NodeOperationError(
							this.getNode(),
							`User Message is too long. Maximum supported length is ${MAX_MESSAGE_LENGTH} characters.`,
						);
					}
					if (assistantMessage.length > MAX_MESSAGE_LENGTH) {
						throw new NodeOperationError(
							this.getNode(),
							`Assistant Message is too long. Maximum supported length is ${MAX_MESSAGE_LENGTH} characters.`,
						);
					}
