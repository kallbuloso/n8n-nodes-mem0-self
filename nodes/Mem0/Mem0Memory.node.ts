...
export class Mem0Memory implements INodeType {
  ...
  async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
    ...
    const searchMode = String(this.getNodeParameter('searchMode', itemIndex, 'balanced') || 'balanced') as 'balanced' | 'strict_facts' | 'legacy'
    ...
    const loadConversationMessages = async (): Promise<any[]> => {
      ...
      return normalized
        .slice(0, maxMessages)
        .reverse()
        .map((entry: any) => toLangchainMessage(entry))
    }
