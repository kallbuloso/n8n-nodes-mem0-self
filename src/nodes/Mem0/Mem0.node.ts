  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (!userId || !agentId) {
          throw new NodeOperationError(this.getNode(), 'User ID and Agent ID are required for this safe profile.')
        }
        ...
        if (operation === 'storeInteraction') {
          ...
          const infer = this.getNodeParameter('infer', i, true) as boolean

          if (!userMessage) {
            throw new NodeOperationError(this.getNode(), 'User Message cannot be empty.')
          }
          if (!assistantMessage) {
            throw new NodeOperationError(this.getNode(), 'Assistant Message cannot be empty.')
          }
          if (userMessage.length > MAX_MESSAGE_LENGTH) {
            throw new NodeOperationError(this.getNode(), `User Message is too long. Maximum supported length is ${MAX_MESSAGE_LENGTH} characters.`)
          }
          if (assistantMessage.length > MAX_MESSAGE_LENGTH) {
            throw new NodeOperationError(this.getNode(), `Assistant Message is too long. Maximum supported length is ${MAX_MESSAGE_LENGTH} characters.`)
          }
        ...
        if (operation === 'searchConversation') {
          ...
          if (!query) {
            throw new NodeOperationError(this.getNode(), 'Search Query cannot be empty.')
          }
          if (query.length > MAX_QUERY_LENGTH) {
            throw new NodeOperationError(this.getNode(), `Search Query is too long. Maximum supported length is ${MAX_QUERY_LENGTH} characters.`)
          }
